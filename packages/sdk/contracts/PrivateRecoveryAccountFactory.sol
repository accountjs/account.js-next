...
contract PrivateRecoveryAccountFactory {
    ...
    function getAddress(address owner, uint256 salt) public view returns (address) {
        bytes memory encodedCall = abi.encodeCall(PrivateRecoveryAccount.initialize, (owner));
        bytes memory encodedAbi = abi.encode(address(accountImplementation), encodedCall);
        bytes memory encodedPacked = abi.encodePacked(type(ERC1967Proxy).creationCode, encodedAbi);
